...
def main():
    ...
    if class_count == 0:
        tf.compat.v1.logging.error('No valid folders of images found at ' + TRAINING_IMAGES_DIR)
        return -1
    # end if
    if class_count == 1:
        tf.compat.v1.logging.error('Only one valid folder of images found at ' + TRAINING_IMAGES_DIR + ' - multiple classes are needed for classification.')
        return -1
    # end if
    ...
    with tf.compat.v1.Session(graph=graph) as sess:
        ...
        train_writer = tf.compat.v1.summary.FileWriter(TENSORBOARD_DIR + '/train', sess.graph)
        validation_writer = tf.compat.v1.summary.FileWriter(TENSORBOARD_DIR + '/validation')
        ...
        for i in range(HOW_MANY_TRAINING_STEPS):
            ...
            if (i % EVAL_STEP_INTERVAL) == 0 or is_last_step:
                ...
                tf.compat.v1.logging.info('%s: Step %d: Train accuracy = %.1f%%' % (datetime.now(), i, train_accuracy * 100))
                tf.compat.v1.logging.info('%s: Step %d: Cross entropy = %f' % (datetime.now(), i, cross_entropy_value))
                ...
                tf.compat.v1.logging.info('%s: Step %d: Validation accuracy = %.1f%% (N=%d)' % (datetime.now(), i, validation_accuracy * 100, len(validation_bottlenecks)))
            # end if
            ...
            if (intermediate_frequency > 0 and (i % intermediate_frequency == 0) and i > 0):
                ...
                tf.compat.v1.logging.info('Save intermediate result to : ' + intermediate_file_name)
        ...
        tf.compat.v1.logging.info('Final test accuracy = %.1f%% (N=%d)' % (test_accuracy * 100, len(test_bottlenecks)))

        if PRINT_MISCLASSIFIED_TEST_IMAGES:
            tf.compat.v1.logging.info('=== MISCLASSIFIED TEST IMAGES ===')
            for i, test_filename in enumerate(test_filenames):
                if predictions[i] != test_ground_truth[i]:
                    tf.compat.v1.logging.info('%70s  %s' % (test_filename, list(image_lists.keys())[predictions[i]]))
                # end if
            # end for
        # end if
...
def create_model_info(architecture):
    ...
    if architecture == 'inception_v3':
        ...
        if (version_string != '1.0' and version_string != '0.75' and version_string != '0.50' and version_string != '0.25'):
            tf.compat.v1.logging.error(""""The Mobilenet version should be '1.0', '0.75', '0.50', or '0.25', but found '%s' for architecture '%s'""", version_string, architecture)
            return None
        # end if
        size_string = parts[2]
        if (size_string != '224' and size_string != '192' and size_string != '160' and size_string != '128'):
            tf.compat.v1.logging.error("""The Mobilenet input size should be '224', '192', '160', or '128', but found '%s' for architecture '%s'""", size_string, architecture)
            return None
        # end if
        if len(parts) == 3:
            is_quantized = False
...
def downloadModelIfNotAlreadyPresent(data_url):
    ...
    if not os.path.exists(filepath):
        ...
        tf.compat.v1.logging.info('Successfully downloaded ' + str(filename) + ', statinfo.st_size = ' + str(statinfo.st_size) + ' bytes')
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:
        ...
        if len(file_list) < 20:
            ...
            tf.compat.v1.logging.warning('WARNING: Folder {} has more than {} images. Some images will never be selected.'.format(dir_name, MAX_NUM_IMAGES_PER_CLASS))
        # end if
...
def add_jpeg_decoding(input_width, input_height, input_depth, input_mean, input_std):
    ...
    resized_image = tf.image.resize(decoded_image_4d, resize_shape_as_int, method=tf.image.ResizeMethod.BILINEAR)
...
def add_input_distortions(flip_left_right, random_crop, random_scale, random_brightness, input_width, input_height,
    ...
    precropped_image = tf.image.resize(decoded_image_4d, precrop_shape_as_int, method=tf.image.ResizeMethod.BILINEAR)
    precropped_image_3d = tf.squeeze(precropped_image, axis=[0])
    cropped_image = tf.image.random_crop(precropped_image_3d, [input_height, input_width, input_depth])
    if flip_left_right:
    ...
    brightness_value = tf.random.uniform(tensor_shape.scalar(), minval=brightness_min, maxval=brightness_max)
    ...
    for label_name, label_lists in image_lists.items():
        for category in ['training', 'testing', 'validation']:
            ...
            if how_many_bottlenecks % 100 == 0:
                tf.compat.v1.logging.info(str(how_many_bottlenecks) + ' bottleneck files created.')
            # end if
# end function
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor, bottleneck_tensor_size, quantize_layer):
    ...
    with tf.compat.v1.name_scope('input'):
        bottleneck_input = tf.compat.v1.placeholder_with_default(bottleneck_tensor, shape=[None, bottleneck_tensor_size], name='BottleneckInputPlaceholder')
        ground_truth_input = tf.compat.v1.placeholder(tf.int64, [None], name='GroundTruthInput')
    # end with
